...
def _faceted(val: dict, audience: str) -> list[dict]:
    ...
    学生视角 facet 作为一句带「学生将看到」标签的引文——教师既读自己的脚本，也读学生将看到的原话，
    ...
    if s:
        out.append({"type": "labeled", "label": "学生将看到", "text": s})
    else:
...
def expand_from_shared(key: str, shared: dict, audience: str = "teacher",
    ...
    if key == "standard":
        ...
        return [{"type": "callout", "kind": "special",
                 "label": f"{shared.get('standard_code', '')}（课标依据）",
